...
const Navbar = () => {
    ...
    const showNavbar = ["/", "/generate"].includes(pathname); // Shoiwing Navbar at specific position
    return (
        showNavbar && <nav className='bg-white w-[80vw] flex justify-between fixed top-10 right-[10vw] rounded-full px-7 py-5 z-10'>
            <div className='logo flex items-center gap-20'>
                ...
                <ul className='flex gap-10'>
                    <Link href={"/"}><li className='hover:font-bold hover:cursor-pointer text-xl'>Templates</li></Link>
                    <Link href={"/"}><li className='hover:font-bold hover:cursor-pointer text-xl'>Marketplace</li></Link>
                    <Link href={"/"}><li className='hover:font-bold hover:cursor-pointer text-xl'>Discover</li></Link>
                    <Link href={"/"}><li className='hover:font-bold hover:cursor-pointer text-xl'>Pricing</li></Link>
                    <Link href={"/"}><li className='hover:font-bold hover:cursor-pointer text-xl'>Learn</li></Link>
                </ul>
            </div>
            <div className='flex gap-2'>
                <button className='login hover:cursor-pointer hover:bg-gray-300 bg-gray-200 p-4 rounded-lg font-bold'>Log in</button>
                <button className='signup hover:cursor-pointer bg-gray-900 p-4 text-white rounded-full transform transition-transform duration-200 hover:scale-105 font-bold'>Sign up free</button>
            </div>
        </nav>
    )
}
